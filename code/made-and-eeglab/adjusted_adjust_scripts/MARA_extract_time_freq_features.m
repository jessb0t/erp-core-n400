% MODIFIED BY JESSICA M. ALEXANDER 03/19/2022
% Downsampling method was throwing error on spectopo call for data at ERP Core standard sampling rate of 1024 Hz
% Modified to ensure that incoming sampling rates of 256 Hz and 1024 Hz would be given factors that were cleanly divisble.
% Additionally modified JPG output to nest in proper folder.
% Changes are commented on lines 23-26, 187, and 190.
...
%downsample (to 100-200Hz) | SECTION MODIFIED BY JESSICA M. ALEXANDER
%factor = max(floor(EEG.srate/100),1);          %commented out this line from the original script
factorExp = ceil(log2(EEG.srate/200))           %added this line
factor = 2^factorExp                            %added this line
...
% plot componetns with bumps and components w/o bumps | SECTION MODIFIED BY JESSICA M. ALEXANDER
figure; hold on;
bumps=subplot(2,1,1);
...
cd interim_ica/         % Added this line to output these jpgs into the proper folder
par_id=strsplit(EEG.setname,'_')
saveas(gcf,[par_id{1} '.jpg'])
cd ..     % Added this line to move us back into the main working directory
