% This script was taken from the MADE pipeline (https://github.com/ChildDevLab/MADE-EEG-preprocessing-pipeline).

% Modifications by George A. Buzzell for the NDCLab Working Group (https://github.com/NDCLab/lab-training/tree/main/eeg-analysis):
    % Lines 62-65: replace commented lines with paths to MADE scripts, EEGLab scripts, and removal of Octave functions in EEGLAB.
    % Lines 92-99: added file paths for raw data, output data, and channel locations file
    % Line 176: modified path for locating .set file
    % Lines 245-246: modified load command
    % Lines 249-253: add code to convert all event markers to strings
    % Lines 279-280: modified command to look up channel locations file

% Modifications by Jessica M. Alexander based, in part, on code from George A. Buzzell shared with the NDCLab Working Group:
    % Lines 101-102: added filepath for event labeling file    
    % Lines 104-169: input specifications (and corrected typo in "response_markers" variable name)
    % Lines 214-222: modified filepath names
    % Lines 231-232, 477, 586, 665-666, 669-669: split "ICs_removed" into two groups: those identified by ADJUST and those identified manually
    % Line 236, 480, 589, 686, 836, 983: added variable to indicate in report whether participant should be rejected based on pipeline criteria
    % Line 238, 690-697: split subject loop into two to enable the addition of manually-identified, artifacted ICs prior to final processing
    % Lines 255-258: added management of subject id
    % Lines 289-291: added call to event labeling file and necessary directory changes
    % Lines 339-346, 350: added verification for empty channels and connected to code for removal of outerlayer channels
    % Line 435: modified existing call to wrap char() function
    % Lines 438-442: added code to save participant-specific channel data information (which is needed in the second loop)
    % Line 469: modified faster_bad_channels data class for consistency in output report
    % Lines 480, 589, 688, 836, 984, 995-999: deleted or commented out total_channels_interpolated variable (can be visually summed in output report)
    % Line 649/651: modified 'MARA_extract_time_freq_features.m', called by adjusted_ADJUST(), to allow run with sample rate of 1024 Hz
    % Lines 693-697: expanded report to include input and output file names, subject id, and sampling rate (including pre-population with placeholders for second loop data points)
    % Lines 700-721: establish setup for second loop, including reading in participant-specific channel data
    % Lines 723-811: manual identification of artifacted ICs not flagged by Adjusted ADJUST
    % Lines 818, 856, 987, 998: replace variable assignments with modification of cell in existing report table
    % Lines 1028-1029: output report for second part of loop (safer than overwriting)
...
stimulus_timeoffset = 26; % stimulus related time offset (in milliseconds). 0 = No time offset
response_timeoffset = 0;    % response related time offset (in milliseconds). 0 = No time offset
stimulus_markers = {'111', '112', '121', '122', '211', '212', '221', '222'};      % enter the stimulus makers that need to be adjusted for time offset
response_markers = {'201', '202'};       % enter the response makers that need to be adjusted for time offset
...
task_event_markers = {'111', '112', '121', '122', '211', '212', '221', '222', '201', '202'}; % enter all the event/condition markers
...
volt_threshold = [-125 125]; % lower and upper threshold (in uV)
...
frontal_channels = {'FP1','FP2','F7','F8'}; % If you set interp_epoch = 1, enter the list of frontal channels to check (see manuscript for detail)
...
reref={'P9', 'P10'}; % Enter electrode name/s or number/s to be used for rereferencing
...
datafile_names=dir([rawdata_location filesep '*.set']);
...
if save_interim_result ==1
    if exist([output_location filesep 'interim_filtered'], 'dir') == 0
        mkdir([output_location filesep 'interim_filtered'])
    end
    if exist([output_location filesep 'interim_ica'], 'dir') == 0
        mkdir([output_location filesep 'interim_ica'])
    end
end
...
ICs_removed_by_adjust=[]; % artifacted ICs identified by ADJUST
ICs_removed_manually=[]; % artifacted ICs identified manually
...
reject_subject=[]; % indication of whether subject should be rejected based on excessive bad data

%% Loop over all data files for labeling, filtering, removing bad channels, running ICA, and identifying bad components
for subject=1:length(datafile_names)
    EEG=[];
    
    fprintf('\n\n\n*** Processing subject %d (%s) - Part 1 ***\n\n\n', subject, datafile_names{subject});
    ...
    % Convert all event.type markers to string (if not already)
    ...
    setname_parts = split(EEG.setname, '_');
    EEG.subject = str2double(setname_parts(1));
    id = EEG.subject;
...
%    EEG = eeg_checkset( EEG );
    ...
    cd(labeling_location)
    event_markers_erpCore_n400
    cd(output_location)
    ...
    chan_vector = zeros(1,EEG.nbchan); % The channel location file provided in ERP CORE has several empty channels; hijacked this code to identify and remove them automatically
    for chan = 1:EEG.nbchan;
        if isempty(EEG.chanlocs(chan).radius);
            chan_vector(1,chan)=1;
        end
    end
    empty_chans = find(chan_vector==1);
    empty_chans = chans_labels([empty_chans]);
    
    [chans,chansidx] = ismember(outerlayer_channel, chans_labels);
    outerlayer_channel_idx = chansidx(chansidx ~= 0);
    channels_to_delete = [outerlayer_channel_idx empty_chans];
    if delete_outerlayer==1
        if isempty(channels_to_delete)==1
    ...
    elseif numel(ref_chan)==0
        ...
        reference_used_for_faster{subject}=char({EEG.chanlocs(ref_chan).labels});
    end
    
    % Save channel data for interpolation in part 2
    cd('interim_filtered/')
    channel_output_name = [num2str(EEG.subject), '_channels_analysed', '.mat'];
    save(channel_output_name,'channels_analysed')
    cd(output_location)
    ...
    if numel(FASTbadChans)==0
        ...
        faster_bad_channels{subject}=num2str(double(FASTbadChans'));
    end
    ...
    if save_interim_result ==1
        if output_format==1
            EEG = eeg_checkset( EEG );
            EEG = pop_editset(EEG, 'setname', strrep(datafile_names{subject}, ext, '_filtered'));
            EEG = pop_saveset( EEG,'filename',strrep(datafile_names{subject}, ext, '_filtered.set'),'filepath', [output_location filesep 'interim_filtered' filesep]); % save .set format
        elseif output_format==2
            save([[output_location filesep 'filtered_data' filesep ] strrep(datafile_names{subject}, ext, '_filtered.mat')], 'EEG'); % save .mat format
        end
    end
    ...
    if size(EEG_copy.icaweights,1) == size(EEG_copy.icaweights,2)
        if save_interim_result==1
            badICs = adjusted_ADJUST(EEG_copy, [[output_location filesep 'interim_ica' filesep] strrep(datafile_names{subject}, ext, '_adjust_report')]);
        else
    ...
    if numel(badICs)==0
        ICs_removed_by_adjust{subject}='0';
        ICs_removed_manually{subject}='0';
    else
        ICs_removed_by_adjust{subject}=num2str(double(badICs));
        ICs_removed_manually{subject}='none';
    end
    ...
    if save_interim_result==1
        if output_format==1
            ...
            EEG = pop_saveset(EEG, 'filename', strrep(datafile_names{subject}, ext, '_ica_data.set'),'filepath', [output_location filesep 'interim_ica' filesep ]); % save .set format
        elseif output_format==2
            save([[output_location filesep 'ica_data' filesep ] strrep(datafile_names{subject}, ext, '_ica_data.mat')], 'EEG'); % save .mat format
        end
    end

    inputfile{subject}=datafile_names{subject};
    outputfile{subject}=EEG.filename;
    samplerate{subject}=EEG.srate;
    reject_subject(subject)=0;
    total_epochs_before_artifact_rejection(subject)=0;
    total_epochs_after_artifact_rejection(subject)=0;

end % end of first subject loop

%% Create the initial report table for all the data files with relevant preprocessing outputs
report_table=table(subject_id', reject_subject', inputfile', outputfile', samplerate', reference_used_for_faster', faster_bad_channels', ica_preparation_bad_channels', length_ica_data', ...
    total_ICs', ICs_removed_by_adjust', ICs_removed_manually', total_epochs_before_artifact_rejection', total_epochs_after_artifact_rejection');
report_table.Properties.VariableNames={'subject_id', 'reject_subject', 'inputfile', 'outputfile', 'samplerate', 'reference_used_for_faster', 'faster_bad_channels', ...
    'ica_preparation_bad_channels', 'length_ica_data', 'total_ICs', 'ICs_removed_by_adjust', 'ICs_removed_manually', 'total_epochs_before_artifact_rejection', 'total_epochs_after_artifact_rejection'};
writetable(report_table, ['MADE_preprocessing_report_', datestr(now,'yyyymmdd'),'_part1.csv']);


%% Set up for second subject loop
% If variables from lines 61-168 are not in workspace, re-run
report_date = '20220408'; %specify date the first loop was run
report_table=readtable(['MADE_preprocessing_report_', report_date,'_part1.csv'], 'Format', 'auto');

datafile_names=dir([output_location filesep 'interim_ica' filesep '*.set']);
datafile_names=datafile_names(~ismember({datafile_names.name},{'.', '..', '.DS_Store'}));
datafile_names={datafile_names.name};
[filepath,name,ext] = fileparts(char(datafile_names{1}));

%% Start second subject loop for removing bad components, epoching, correcting baseline, removing residual artifacts, and re-referencing
for subject=1:length(datafile_names)
    EEG=[];    
    fprintf('\n\n\n*** Processing subject %d (%s) - Part 2 ***\n\n\n', subject, datafile_names{subject});
    EEG = pop_loadset('filename', datafile_names{subject}, 'filepath', [output_location filesep 'interim_ica' filesep]);
    EEG = eeg_checkset(EEG);

    cd('interim_filtered/')
    channelData = [num2str(EEG.subject), '_channels_analysed', '.mat']
    load(channelData)
    channels_analysed = channels_analysed;
    cd(output_location)

% Specify manually-identified bad components and update EEG.reject.gcompreject
    if EEG.subject==1
        moreBadICs = [9];
    elseif EEG.subject==2
        moreBadICs = [];
    elseif EEG.subject==3
        moreBadICs = [1 8];
    elseif EEG.subject==4
        moreBadICs = [6];
    elseif EEG.subject==5
        moreBadICs = [1];
    elseif EEG.subject==6
        moreBadICs = [3];
    elseif EEG.subject==7
        moreBadICs = [1];
    elseif EEG.subject==8
        moreBadICs = [6];
    elseif EEG.subject==9
        moreBadICs = [5];
    elseif EEG.subject==10
        moreBadICs = [1 4];
    elseif EEG.subject==11
        moreBadICs = [];
    elseif EEG.subject==12
        moreBadICs = [];
    elseif EEG.subject==13
        moreBadICs = [];
    elseif EEG.subject==14
        moreBadICs = [3];
    elseif EEG.subject==15
        moreBadICs = [];
    elseif EEG.subject==16
        moreBadICs = [];
    elseif EEG.subject==17
        moreBadICs = [4];
    elseif EEG.subject==18
        moreBadICs = [1 10];
    elseif EEG.subject==19
        moreBadICs = [];
    elseif EEG.subject==20
        moreBadICs = [];
    elseif EEG.subject==21
        moreBadICs = [1 2];
    elseif EEG.subject==22
        moreBadICs = [1];
    elseif EEG.subject==23
        moreBadICs = [4];
    elseif EEG.subject==24
        moreBadICs = [];
    elseif EEG.subject==25
        moreBadICs = [18];
    elseif EEG.subject==26
        moreBadICs = [];
    elseif EEG.subject==27
        moreBadICs = [];
    elseif EEG.subject==28
        moreBadICs = [1 4];
    elseif EEG.subject==29
        moreBadICs = [3];
    elseif EEG.subject==30
        moreBadICs = [1];
    elseif EEG.subject==31
        moreBadICs = [];
    elseif EEG.subject==32
        moreBadICs = [4];
    elseif EEG.subject==33
        moreBadICs = [1 4 8];
    elseif EEG.subject==34
        moreBadICs = [];
    elseif EEG.subject==35
        moreBadICs = [];
    elseif EEG.subject==36
        moreBadICs = [1 2];
    elseif EEG.subject==37
        moreBadICs = [];
    elseif EEG.subject==38
        moreBadICs = [];
    elseif EEG.subject==39
        moreBadICs = [3];
    elseif EEG.subject==40
        moreBadICs = [5];
    end

    report_table.ICs_removed_manually(find(report_table.subject_id==EEG.subject), 1) = {num2str(moreBadICs)};

    for ic=1:length(moreBadICs)
        EEG.reject.gcompreject(1, moreBadICs(ic))=1;
        EEG = eeg_checkset(EEG);
    end
...
%% STEP 11: Remove artifacted ICA components from data
    ...
    if numel(ICs2remove)==report_table.total_ICs(find(report_table.subject_id==EEG.subject), 1)
        all_bad_ICs=1;
        warning(['No usable data for datafile', datafile_names{subject}]);
        if output_format==1
            EEG = eeg_checkset(EEG);
    ...
    %% STEP 12: Segment data into epochs
    if epoch_data==1
    ...
    report_table.total_epochs_before_artifact_rejection(find(report_table.subject_id==EEG.subject), 1) = EEG.trials;
    ...
    if all_bad_epochs==1
        ...
        report_table.total_epochs_after_artifact_rejection(find(report_table.subject_id==EEG.subject), 1)=EEG.trials;
    end
    ...
    %else
    %    report_table.total_channels_interpolated(find(report_table.subject_id==EEG.subject), 1) = numel(FASTbadChans)+ numel(ica_prep_badChans);
    %end
    ...
    if output_format==1
        EEG = eeg_checkset(EEG);
        EEG = pop_editset(EEG, 'setname',  strrep(datafile_names{subject}, ext, '_processed'));
        EEG = pop_saveset(EEG, 'filename', strrep(datafile_names{subject}, ext, '_processed.set'),'filepath', [output_location filesep 'processed_data' filesep ]); % save .set format
    elseif output_format==2
...
end % end of second subject loop

%% Update the report table with second loop data
writetable(report_table, ['MADE_preprocessing_report_', datestr(now,'yyyymmdd'),'_part2.csv']);
